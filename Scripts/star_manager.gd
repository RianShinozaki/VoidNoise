...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
			...
			_star.direction = current_direction
			current_direction += PI/12
...
func _draw() -> void:
	...
	for _star in stars:
		if _star is Comet:
			draw_circle(_star.position, _star.radius, Color.AQUA, true)
			@warning_ignore("integer_division")
			draw_texture(comet_gradient, _star.position - Vector2(comet_gradient.get_width()/2, comet_gradient.get_height()/2))
		else:
			draw_circle(_star.position, _star.radius, Color.WHITE, true)
			@warning_ignore("integer_division")
